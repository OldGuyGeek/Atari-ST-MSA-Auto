...
$OpenFileDialog.filter = "ST Disk|*.st|STX Disk|*.stx|MSA Disk|*.MSA|Zips|*.zip"
...
$FileExtensionType = [System.IO.Path]::GetExtension($Global:SelectedFile)
...
# If it was a zip flie, extract .zip to new directory

if ($FileExtensionType -eq ".ZIP") { 
    # extract Zip file to directory
    Expand-Archive -LiteralPath $Global:SelectedFile -DestinationPath $FullDestinationFolder
   Invoke-Item .
}
    ...
    Invoke-Item -path $msainput.DestinationFolder
}
